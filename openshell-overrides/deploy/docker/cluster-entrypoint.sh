...
# ---------------------------------------------------------------------------
# Offline preload: stage image tar archives for k3s auto-import.
# ---------------------------------------------------------------------------
# k3s automatically imports tar archives found under:
#   /var/lib/rancher/k3s/agent/images/*.tar
# at startup. By copying host-provided tar files here before launching k3s,
# we avoid first-boot pulls from external registries in air-gapped setups.
stage_k3s_preload_tars() {
    [ -z "${K3S_PRELOAD_IMAGE_TARS:-}" ] && return 0

    local image_dir="/var/lib/rancher/k3s/agent/images"
    mkdir -p "$image_dir"

    local old_ifs="$IFS"
    IFS=','
    for src in $K3S_PRELOAD_IMAGE_TARS; do
        src=$(echo "$src" | xargs)
        [ -z "$src" ] && continue
        if [ ! -f "$src" ]; then
            echo "Warning: preload tar not found: $src"
            continue
        fi

        local base
        base=$(basename "$src")
        local dst="$image_dir/$base"

        # Avoid recopying when a matching file already exists.
        if [ -f "$dst" ] && [ "$(stat -c '%s' "$dst" 2>/dev/null || echo 0)" = "$(stat -c '%s' "$src" 2>/dev/null || echo -1)" ]; then
            echo "Preload tar already staged: $dst"
            continue
        fi

        echo "Staging k3s preload tar: $src -> $dst"
        cp -f "$src" "$dst"
    done
    IFS="$old_ifs"
}
