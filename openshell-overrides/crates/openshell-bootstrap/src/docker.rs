...
pub async fn ensure_container(
    ...
    let mut effective_k3s_tars: Vec<String> = k3s_image_tars.to_vec();
    if effective_k3s_tars.is_empty()
        && let Some(raw) = env_non_empty("OPENSHELL_K3S_IMAGE_TARS")
    {
        effective_k3s_tars.extend(
            raw.split(',')
                .map(str::trim)
                .filter(|s| !s.is_empty())
    ...
    let mut preload_targets = Vec::new();
    for (idx, host_tar) in effective_k3s_tars.iter().enumerate() {
        let host_tar = host_tar.trim();
        if host_tar.is_empty() {
            continue;
        }
        let base_name = Path::new(host_tar)
            .file_name()
            .and_then(|s| s.to_str())
            .filter(|s| !s.is_empty())
            .unwrap_or("image.tar");
        let container_tar = format!("/opt/openshell/offline-images/{idx:02}-{base_name}");
        binds.push(format!("{host_tar}:{container_tar}:ro"));
        preload_targets.push(container_tar);
    }
    ...
    if !preload_targets.is_empty() {
        env_vars.push(format!(
            "K3S_PRELOAD_IMAGE_TARS={}",
            preload_targets.join(",")
        ));
    }
