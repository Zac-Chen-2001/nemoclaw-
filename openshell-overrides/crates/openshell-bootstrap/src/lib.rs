...
pub struct DeployOptions {
    ...
    pub recreate: bool,
    /// Host paths to image tar archives that should be mounted into the
    /// cluster container and imported into k3s/containerd before k3s starts.
    pub k3s_image_tars: Vec<String>,
}
...
impl DeployOptions {
    ...
    /// Set k3s image tar archives to preload before k3s startup.
    #[must_use]
    pub fn with_k3s_image_tars<I, S>(mut self, tars: I) -> Self
    where
        I: IntoIterator<Item = S>,
        S: Into<String>,
    {
        self.k3s_image_tars = tars.into_iter().map(Into::into).collect();
        self
    }
}
    ...
    F: FnMut(String) + Send + 'static,
{
    ...
    let k3s_image_tars = options.k3s_image_tars;
